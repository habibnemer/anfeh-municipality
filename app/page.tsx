  ...
  return (
    <>
      ...
      <section className="relative w-full min-h-[100svh] flex flex-col justify-end overflow-hidden" style={{background:'#1A2E4A'}}>
        ...
        <div className="relative z-10 container-site pb-20 pt-36">
          <div className="max-w-3xl">
            <p className="text-[10px] tracking-[0.4em] uppercase mb-6 animate-fade-up" style={{color:'rgba(255,255,255,0.4)'}}>Anfeh · Koura · Lebanon</p>
            <h1 className="font-serif font-semibold text-white mb-6 animate-fade-up delay-100" style={{fontFamily:'var(--font-cormorant,Georgia,serif)',fontSize:'clamp(3.5rem,9vw,7rem)',lineHeight:1,letterSpacing:'-0.02em'}}>
              ANFEH
            </h1>
            <p className="font-serif mb-4 animate-fade-up delay-200" style={{fontFamily:'var(--font-cormorant,Georgia,serif)',fontSize:'clamp(1.25rem,2.5vw,1.75rem)',color:'rgba(255,255,255,0.8)'}}>
            ...
            <p className="text-sm md:text-base leading-relaxed max-w-xl mb-10 animate-fade-up delay-300" style={{color:'rgba(255,255,255,0.55)'}}>
              Discover the municipality, people, heritage and coastline that make Anfeh unique.
            </p>
            <div className="flex flex-wrap gap-4 animate-fade-up delay-400">
              <Link href="/discover" className="btn-primary">Explore Anfeh <ArrowRight size={15} /></Link>
              <Link href="/services" className="btn-secondary" style={{borderColor:'rgba(255,255,255,0.4)',color:'white'}}>Municipal Services</Link>
            </div>
          </div>
        </div>
      ...
      <section className="section-padding" style={{background:'var(--color-warm-white)'}}>
        <div className="container-site">
          <div className="flex items-end justify-between gap-4 mb-8 md:mb-12">
            <SectionHeader label="For Residents" title="Municipal Services" subtitle="Access all services in one place." />
            <Link href="/services" className="btn-ghost whitespace-nowrap hidden md:inline-flex">All Services <ArrowRight size={14} /></Link>
          </div>
          ...
          <div className="mt-4 md:hidden">
            <Link href="/services" className="btn-ghost w-full justify-center border" style={{borderColor:'var(--color-stone)'}}>All Services <ArrowRight size={14} /></Link>
          </div>
        </div>
      </section>
        ...
        <div className="container-site py-14">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex items-start gap-5">
              ...
              <div>
                <h3 className="font-serif text-2xl md:text-3xl mb-2" style={{fontFamily:'var(--font-cormorant,Georgia,serif)',color:'var(--color-navy)'}}>
                  See something that needs attention?
                </h3>
                <p className="text-sm max-w-md" style={{color:'var(--color-muted)'}}>
                  Help us keep Anfeh safe, clean and connected. Report road damage, lighting issues, waste and more.
                </p>
              </div>
            </div>
      ...
      <section className="section-padding">
        <div className="container-site">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <SectionHeader label="What We're Building" title="Municipal Projects" />
            <Link href="/municipality/projects" className="btn-ghost whitespace-nowrap">All Projects <ArrowRight size={14} /></Link>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    {project.tags.slice(0,2).map((t)=>(
                      <span key={t} className="text-[10px] tracking-wide uppercase" style={{color:'var(--color-limestone)'}}>{t}</span>
                    ))}
                  </div>
      ...
      <section className="section-padding" style={{background:'var(--color-navy)'}}>
        <div className="container-site">
          <div className="flex items-end justify-between gap-4 mb-8 md:mb-12">
            <SectionHeader label="Heritage & Coastline" title="Discover Anfeh" light />
            ...
            {discoverItems.map((item) => (
              <Link key={item.id} href={`/discover?category=${item.category}`} className="group block relative overflow-hidden" style={{height:220}}>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 md:p-5">
                  ...
                  <p className="font-serif text-base md:text-xl text-white leading-tight" style={{fontFamily:'var(--font-cormorant,Georgia,serif)'}}>{item.title}</p>
                </div>
              </Link>
            ))}
          </div>
          <div className="mt-4 md:hidden">
            <Link href="/discover" className="w-full justify-center text-sm py-3 flex items-center gap-2" style={{color:'rgba(255,255,255,0.6)'}}>
              Explore All of Anfeh <ArrowRight size={14} />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding" style={{background:'var(--color-warm-white)'}}>
        <div className="container-site">
          <div className="flex items-end justify-between gap-4 mb-8 md:mb-12">
            <SectionHeader label="Latest" title="Municipal News" />
            <Link href="/news" className="btn-ghost whitespace-nowrap">All News <ArrowRight size={14} /></Link>
          </div>
            ...
            {[featuredNews, ...sideNews].filter(Boolean).slice(0,3).map((article)=> article && (
              <Link key={article.id} href={`/news/${article.slug}`} className="bg-white flex gap-4 p-4 group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[10px]" style={{color:'var(--color-limestone)'}}>{new Date(article.date).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}</span>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="hidden md:grid grid-cols-1 lg:grid-cols-3 gap-px" style={{background:'var(--color-stone)'}}>
            {featuredNews && (
              <Link href={`/news/${featuredNews.slug}`} className="lg:col-span-2 bg-white p-6 md:p-8 flex flex-col group">
                ...
                <h3 className="font-serif text-2xl md:text-3xl mb-3" style={{fontFamily:'var(--font-cormorant,Georgia,serif)',color:'var(--color-navy)'}}>{featuredNews.title}</h3>
                ...
                <div className="flex items-center justify-between mt-5 pt-5" style={{borderTop:'1px solid var(--color-stone)'}}>
                  <span className="text-xs" style={{color:'var(--color-limestone)'}}>{new Date(featuredNews.date).toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}</span>
      ...
      <section className="section-padding" style={{background:'var(--color-cream)'}}>
        <div className="container-site">
          <div className="flex items-end justify-between gap-4 mb-8 md:mb-12">
            <SectionHeader label="Community" title="Upcoming Events" />
            <Link href="/events" className="btn-ghost whitespace-nowrap">All Events <ArrowRight size={14} /></Link>
          </div>
            ...
            {upcomingEvents.map((event)=>(
              <div key={event.id} className="bg-white p-5 md:p-6 flex flex-row md:flex-col gap-4">
                <div className="w-14 h-14 flex flex-col items-center justify-center text-white flex-none" style={{background:'var(--color-navy)'}}>
                  <span className="font-serif font-semibold text-lg leading-none" style={{fontFamily:'var(--font-cormorant,Georgia,serif)'}}>{new Date(event.date).getDate()}</span>
                  <span className="text-[9px] tracking-widest uppercase" style={{color:'rgba(255,255,255,0.5)',marginTop:2}}>{new Date(event.date).toLocaleDateString('en-GB',{month:'short'})}</span>
                </div>
                <div className="flex-1 min-w-0">
      ...
      <section className="section-padding" style={{background:'var(--color-navy)'}}>
        <div className="container-site text-center">
          <p className="text-[10px] tracking-[0.4em] uppercase mb-5" style={{color:'rgba(255,255,255,0.3)'}}>Anfeh Municipality</p>
          <h2 className="font-serif text-white mb-4" style={{fontFamily:'var(--font-cormorant,Georgia,serif)',fontSize:'clamp(2rem,6vw,3.5rem)'}}>
            Built for every resident of Anfeh
          </h2>
          <p className="text-sm md:text-base max-w-md mx-auto mb-8" style={{color:'rgba(255,255,255,0.6)'}}>
            Municipal services, issue reporting, events and community — all in one place.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link href="/services" className="btn-primary w-full sm:w-auto justify-center">Municipal Services <ArrowRight size={15} /></Link>
            <Link href="/community" className="btn-secondary w-full sm:w-auto justify-center" style={{borderColor:'rgba(255,255,255,0.3)',color:'white'}}>Community</Link>
          </div>
        </div>
      </section>
    </>
